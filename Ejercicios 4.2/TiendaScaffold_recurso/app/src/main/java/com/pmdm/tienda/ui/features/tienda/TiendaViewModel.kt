...
class TiendaViewModel @Inject constructor(
    ...
    val clienteState by mutableStateOf(getCliente(login))
    ...
    private var mostrarFavorito=false
    ...
    constructor(parcel: Parcel) : this(
        TODO("articuloRepository"),
        TODO("clienteRepository"),
        TODO("pedidoRepository")
    ) {
        mostrarFavorito = parcel.readByte() != 0.toByte()
    }
        ...
        when (tiendaEvent) {
            ...
            is TiendaEvent.OnClickFavorito -> {
                ...
                if (posicion != -1) {
                   ...
                   if(mostrarFavorito) _articulosState = getArticulos(clienteState.favoritos!!) ?: getArticulos()
                }
            }
    ...
    override fun writeToParcel(parcel: Parcel, flags: Int) {
        parcel.writeByte(if (mostrarFavorito) 1 else 0)
    }
    ...
    companion object CREATOR : Parcelable.Creator<TiendaViewModel> {
        override fun createFromParcel(parcel: Parcel): TiendaViewModel {
            return TiendaViewModel(parcel)
        }

        override fun newArray(size: Int): Array<TiendaViewModel?> {
